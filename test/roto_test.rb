...
class TestRoto < Minitest::Test
  ...
  def test_that_finder_can_find_jpgs
    filetypes=['jpg']
    photos = @roto.find_photos(@fixtures_path, filetypes)
  ...
  def test_that_mover_can_move
    files = @roto.find_photos(@fixtures_path, @filetypes)
  ...
  def test_that_copier_can_copy
    ...
    assert_equal ["test.mp4", "test.jpg", "test.png"], @roto.find_photos(@moved_files_path, @filetypes).map {|path| File.basename path}
    assert_equal ["test.mp4", "test.jpg", "test.png"], @roto.find_photos(@fixtures_path, @filetypes).map {|path| File.basename path}
  end
